...
import { askClaude, extractJson, bioBlock, usageCost, CHEAP_MODEL } from "./_lib/claude.js";
...
// Бесплатная механическая чистка транскрипта перед отправкой в модель:
// таймкоды, служебные пометки ([Музыка], [Аплодисменты]) и повторы строк
// автосубтитров — это 20-30% объёма, за чтение которых нечего платить.
function cleanTranscript(text) {
  ...
  for (let line of lines) {
    line = line
      ...
      .replace(/\[(?:Музыка|Аплодисменты|Смех|Music|Applause|Laughter)\]/gi, " ")
      .replace(/\s+/g, " ")
      .trim();
    if (!line) continue;
    if (line === prev) continue; // автосубтитры часто дублируют строки
    out.push(line);
    prev = line;
  }
  return out.join(" ");
...
// Выжимка — обычный текст (не JSON): длинный конспект с кавычками и списками
// в JSON-строке регулярно ломал парсинг.
const DIGEST_SYSTEM = `Ты сжимаешь транскрипт видео в плотную выжимку для автора YouTube-канала. Задача — удалить ВСЮ воду (приветствия, повторы, самопиар, лирику, переходы) и сохранить ВСЁ содержательное: конкретные приёмы, шаги, цифры, примеры, названия, выводы, принципы. Ничего не выдумывай — только то, что реально есть в тексте.

Формат ответа — обычный текст, без JSON и преамбул:
ТЕМА: [одно предложение — о чём видео на самом деле]

СУТЬ ПО БЛОКАМ:
1. [название смыслового блока]
   — тезис/приём с конкретикой
   — ...
2. ...

ГЛАВНЫЕ ВЫВОДЫ И ПРИНЦИПЫ:
— [вывод, применимый на практике]
— ...

Объём выжимки: примерно 10-15% исходного текста. Пиши по-русски, даже если транскрипт на другом языке.`;
...
async function analyze(body, usage) {
  ...
  const manual = cleanTranscript(manualTranscript || "");
  const videoId = parseVideoId(url);
  ...
  // Режим «Выжимка»: дешёвая модель удаляет воду и отдаёт конспект обычным
  // текстом. Экономика: чтение Haiku ~в 3 раза дешевле Sonnet, а выжимку
  // потом можно скормить в глубокий разбор — дорогая модель прочтёт уже
  // маленький текст.
  if (mode === "digest") {
    if (!rawTranscript) throw new Error("Для выжимки нужен текст: вставь субтитры/транскрипт в поле ниже");
    const { text: digestSource, truncated: digestTruncated } = truncateTranscript(rawTranscript);
    const digestText = await askClaude({
      usage,
      model: CHEAP_MODEL,
      system: DIGEST_SYSTEM,
      user: `${bioBlock(channelBio)}${meta ? `Видео: ${meta.title} (${meta.channel}, ${meta.duration})\n\n` : ""}Транскрипт${digestTruncated ? " (усечён: начало и конец)" : ""}:\n${digestSource}\n\nСделай выжимку.`,
      maxTokens: 4000,
    });
    return {
      meta,
      digest: digestText.trim(),
      transcriptStatus: manual ? "manual" : digestTruncated ? "truncated" : "full",
    };
  }
...
// Без этого флага Vercel БУФЕРИЗУЕТ ответ Node-функции целиком: res.write
// ничего не отправляет клиенту до самого конца, heartbeat не работает,
// и соединение на долгих анализах умирает так же, как без него.
