...
ОБЯЗАТЕЛЬНО: оба заголовка должны содержать формулировку запроса или её близкий естественный вариант, сохраняя поисковый интент.

`;
}

export default jsonHandler(async (body, usage) => {
  const { topic, script, links, current, instruction, channelBio, topicResearch } = body;
